...
namespace SGA.Tests.Application.Services
{
    ...
    public class PromotionServiceFixedTests
    {
        ...
        public async Task CheckEligibilityAsync_TeacherNotFound_ReturnsNotEligible()
        {
            ...
            // Configurar el mock para devolver null de forma segura con respecto a la nulabilidad
            _mockTeacherRepository
                .Setup(repo => repo.GetByIdAsync(teacherId))
        ...
        public async Task CheckEligibilityAsync_TeacherEligible_ReturnsEligibleResult()
        {
            ...
            // Configurar el mock de Teacher para que el método CheckPromotionEligibility retorne el resultado esperado
            var eligibilityResult = new Dictionary<string, bool>
            {
                { "TimeInRank", true },
                { "Works", true },
